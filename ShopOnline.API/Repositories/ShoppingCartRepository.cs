...
namespace ShopOnline.API.Repositories
{
    public class ShoppingCartRepository : IShoppingCartRepository
    {
        ...
        public async Task<CartItem> AddItem(CartItemToAddDto cartIteemToAdd)
        {
            if (await CartItemExists(cartIteemToAdd.Id, cartIteemToAdd.ProductId))
            {
                var item = await (from product in _context.Products
                                  where product.Id == cartIteemToAdd.ProductId
                                  select new CartItem
                                  {
                                      CartId = cartIteemToAdd.Id,
                                      ProductId = product.Id,
                                      Qty = cartIteemToAdd.Qty,
